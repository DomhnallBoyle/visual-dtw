"""Evaluating the CFE based on pose and lighting
"""
...
class MarkDetector:
    ...
    def __init__(self, saved_model='models'):
        """Initialization"""
    ...
    def box_in_image(box, image):
        ...
        rows = image.shape[0]
        cols = image.shape[1]
        return box[0] >= 0 and box[1] >= 0 and box[2] <= cols and box[
            3] <= rows
...
def main():
    # cam = Cam(debug=True)
    # for video_path in cam.record_loop():
    #     create_template(video_path, debug=True)
    ...
    while True:
        ...
        if ret == True:
            ...
            for facebox in faceboxes:
                ...
                x1, x2 = draw_annotation_box(img, rotation_vector,
                ...
                cv2.putText(img, str(ang1), tuple(p1), font, 2,
                            (128, 255, 255), 3)
                cv2.putText(img, str(ang2), tuple(x1), font, 2,
                            (255, 255, 128), 3)
            cv2.imshow('img', img)
